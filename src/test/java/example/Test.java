...
public class Test {
	...
	public void setUp() throws Exception {
		...
		if (null != androidPackage) {
			...
			chromeOptions.setExperimentalOptions("androidPackage",
					androidPackage);
		...
		driver = new Augmenter().augment(new RemoteWebDriver(new URL(
				WEBDRIVER_SERVER_URL), caps));
	...
	public void tearDown() throws Exception {
		try {
			...
			submitPerformanceResult("Test.testGoogleSearch",
	...
	void submitPerformanceResult(String name, List<LogEntry> perfLogEntries)
		...
		for (LogEntry entry : perfLogEntries) {
			...
			/*System.out.println(
			  devToolsMessage.getString("method") + " " +
			  message.getString("webview"));*/
			devToolsLog.put(devToolsMessage);
		}
		byte[] screenshot = null;
		if (null == androidPackage) { // Chrome on Android does not yet support
										// screenshots
			screenshot = ((TakesScreenshot) driver)
					.getScreenshotAs(OutputType.BYTES);
		}
		String resultUrl = new WebPageTest(new URL("http://192.168.88.7:8888/"),
				"Test", name).submitResult(devToolsLog, screenshot);
